...
class Norm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return rms_ball_proj(x, self.eps)
...
class Block(nn.Module):
    def __init__(self, d_model: int, n_head: int, hidden_dim: int, block_size: int, prenorm: str = 'rmsnorm', rope_base: float = 10000.0):
        super().__init__()
...
class GPT(nn.Module):
    def __init__(self, cfg: GPTConfig):
        ...
        self.blocks = nn.ModuleList([
            Block(cfg.d_model, cfg.n_head, cfg.hidden_dim, cfg.block_size, cfg.prenorm, cfg.rope_base)
    ...
    def generate(self, idx: torch.Tensor, max_new_tokens: int, temperature: float = 1.0, top_k: int = 40):
...
class BatchSource:
    def __init__(self, train: torch.Tensor, val: torch.Tensor, block_size: int, batch_size: int, device: torch.device):
    ...
    def get(self, split: str):
        ...
        ix = torch.randint(0, data.numel() - self.block_size - 1, (self.batch_size, 1), device=self.device)
