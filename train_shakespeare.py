...
from gpt import GPT, GPTConfig, CharDataset, BatchSource, maybe_download_tiny_shakespeare
from scionc import ScionC, SpectralLMO, ColNormLMO, RowNormLMO, init_colnorm_, init_rownorm_, init_semiorthogonal_
...
def cosine_lr(step: int, max_steps: int, warmup: int, lr: float, min_lr: float) -> float:
    if step < warmup:
        return lr * (step + 1) / max(1, warmup)
    if step >= max_steps:
        return min_lr
    t = (step - warmup) / max(1, max_steps - warmup)
    return min_lr + 0.5 * (1.0 + math.cos(math.pi * t)) * (lr - min_lr)


@torch.inference_mode()
def estimate_loss(model: GPT, source: BatchSource, eval_iters: int, amp_dtype: torch.dtype | None, device: torch.device):
    out = {}
    was_training = model.training
    model.eval()
    ctx = torch.autocast(device_type='cuda', dtype=amp_dtype) if amp_dtype is not None else torch.no_grad()
    with ctx:
...
def build_scionc(model: GPT, args, device: torch.device) -> ScionC:
    ...
    lmo_hidden = SpectralLMO(radius=args.rho_hidden, steps=args.pe_steps, work_dtype=work_dtype)
    ...
    return ScionC(groups, lr=args.lr, beta2=args.beta2, phi=args.phi, eta=args.eta, cwd=args.cwd)


def save_checkpoint(path: Path, model: GPT, dataset: CharDataset, args):
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save({'model': model.state_dict(), 'model_cfg': asdict(model.cfg), 'chars': dataset.chars, 'args': vars(args)}, path)
...
def maybe_compile(model: GPT, source: BatchSource, args, amp_dtype: torch.dtype | None, device: torch.device):
    ...
    if args.compile and hasattr(torch, 'compile'):
        ...
        ctx = torch.autocast(device_type='cuda', dtype=amp_dtype) if amp_dtype is not None else torch.enable_grad()
        with ctx:
...
def train(args):
    ...
    for step in range(args.max_iters):
        lr = cosine_lr(step, args.max_iters, args.warmup_iters, args.lr, args.min_lr)
        for group in opt.param_groups:
            group['lr'] = lr

        if step % args.eval_interval == 0 or step == args.max_iters - 1:
            losses = estimate_loss(model, source, args.eval_iters, amp_dtype, device)
    ...
    y = raw_model.generate(x, max_new_tokens=args.sample_tokens, temperature=args.temperature, top_k=args.top_k)
    print('\n--- sample ---\n')
...
def sample(args):
    ...
    y = model.generate(x, max_new_tokens=args.sample_tokens, temperature=args.temperature, top_k=args.top_k)
