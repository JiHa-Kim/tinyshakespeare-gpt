...
class ColNormLMO:
    ...
    def __call__(self, w: torch.Tensor) -> torch.Tensor:
        d_out = w.size(0)
        return w.div_(
            torch.linalg.vector_norm(w, dim=0, keepdim=True).clamp_min_(self.eps)
        ).mul_(-self.radius * math.sqrt(d_out))
...
class RowNormLMO:
    ...
    def __call__(self, w: torch.Tensor) -> torch.Tensor:
        d_in = w.size(1)
        return w.div_(
            torch.linalg.vector_norm(w, dim=1, keepdim=True).clamp_min_(self.eps)
        ).mul_(-self.radius / math.sqrt(d_in))
...
class SpectralLMO:
    ...
    def __call__(self, v: torch.Tensor) -> torch.Tensor:
        ...
        if self.input_like:
            scale = max(1.0, scale)
        u = polar_express_uvt(
            v, steps=self.steps, eps=self.eps, work_dtype=self.work_dtype
        )
...
def scion_transfer_lr(
    base_lr: float, mT: float = 1.0, mL: float = 1.0, alpha: float = 0.5
) -> dict[str, float]:
    return {
...
class Scion(LionKCCWDPA):
    """
    Scion specialization.

    Important defaults for this repo:
      - nesterov ON by default
      - weight decay OFF by default
