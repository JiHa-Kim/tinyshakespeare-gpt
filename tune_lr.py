        ...
        if torch.is_tensor(v):
            ...
            out[k] = [
                ...
                if isinstance(x, dict)
                    ...
                    x.detach().to(x.device if device is None else device).clone()
...
def prepare_runtime(base_args):
    ...
    device = torch.device(
        base_args.device or ("cuda" if torch.cuda.is_available() else "cpu")
    )
    amp_dtype = (
        torch.bfloat16
    ...
    *,
    ...
    for step in range(steps):
        ...
        if completed_steps in snapshot_steps:
            snapshot_device = (
                torch.device("cpu") if getattr(opt, "_cpu_snapshots", False) else None
            )
            snapshots[completed_steps] = {
                "model": clone_state_dict(
                    raw_model.state_dict(), device=snapshot_device
                ),
...
def screen_candidate(exp2_lr, args, dataset, device, amp_dtype, seed):
    ...
    print(
        f"=== screen | {run_args.prenorm} | seed {seed} | 2**{exp2_lr:.2f} = {lr:.3e} ==="
    )
    metrics = run_loop(
        model=model,
        raw_model=raw_model,
    ...
    for _ in range(args.screen_max_expansions + 1):
        for exp2_lr in exp2_grid(exp2_min, exp2_max, args.coarse_step):
            if exp2_lr not in cache:
                cache[exp2_lr] = screen_candidate(
                    exp2_lr, screen_args, dataset, device, amp_dtype, screen_seed
                )
        ...
        if stable:
            best = min(
                stable,
                key=lambda r: score_key(
                    r["screen_score"], r["final_val"], r["exp2_lr"]
                ),
            )
            top = max(stable, key=lambda r: r["exp2_lr"])
            low = min(stable, key=lambda r: r["exp2_lr"])
            expanded = False
            if round(top["exp2_lr"], 10) == round(
                exp2_max, 10
            ) and score_with_tolerance(
                best["screen_score"], top["screen_score"], args.edge_tol
            ):
                exp2_max += args.coarse_step * args.expand_points
                expanded = True
            elif round(low["exp2_lr"], 10) == round(
                exp2_min, 10
            ) and score_with_tolerance(
                best["screen_score"], low["screen_score"], args.edge_tol
            ):
...
def adaptive_zoom_screen(
    cache, screen_args, dataset, device, amp_dtype, screen_seed, args
):
    for _ in range(args.zoom_max_rounds):
        ...
        best = min(
            stable,
            key=lambda r: score_key(r["screen_score"], r["final_val"], r["exp2_lr"]),
        )
...
def shortlist_from_screen(
    rows, shortlist_k: int, keep_highest_k: int, aggressive_tol: float
):
    stable = [r for r in rows if r["stable"] and math.isfinite(r["screen_score"])]
    if not stable:
    ...
    if keep_highest_k > 0 and stable:
        ...
        aggressive = [
            r
            for r in stable
            if score_with_tolerance(best_score, r["screen_score"], aggressive_tol)
        ]
        aggressive.sort(
            key=lambda r: (-r["exp2_lr"], r["screen_score"], r["final_val"])
        )
...
def run_constant_candidate(stage, exp2_lr, run_args, dataset, device, amp_dtype, seed):
    ...
    print(
        f"=== {stage} | {args.prenorm} | seed {seed} | 2**{exp2_lr:.2f} = {lr:.3e} ==="
    )
    metrics = run_loop(
        model=model,
        raw_model=raw_model,
...
def choose_branch_finalists(rows, final_lr_k: int, aggressive_tol: float):
    ...
    if final_lr_k > 1:
        ...
        aggressive = [
            r
            for r in stable
            if score_with_tolerance(best_score, r["race_score"], aggressive_tol)
        ]
...
def pick_extra_decay_frac(decay_fracs, evaluated_rows, close_rel_tol):
    ...
    rows = sorted(
        ...
        key=lambda r: score_key(r["score_final"], r["score_avg"], r["exp2_lr"]),
    )
    best = rows[0]
    ...
    if len(rows) >= 2 and score_with_tolerance(
        rows[0]["score_final"], rows[1]["score_final"], close_rel_tol
    ):
        remaining = [
            i for i, df in enumerate(decay_fracs) if round(df, 10) not in evaluated_rows
        ]
        if remaining:
            remaining.sort(
                key=lambda i: (
                    abs(i - best_idx),
                    abs(decay_fracs[i] - decay_fracs[best_idx]),
                )
            )
...
def run_branch_family_adaptive(
    exp2_lr,
    branch_args,
    dataset,
    device,
    ...
    print(
        f"=== branch trunk | {trunk_args.prenorm} | seed {seed} | 2**{exp2_lr:.2f} = {lr:.3e} ==="
    )
    ...
    tail_args.lr = lr
    raw_tail, tail_model, tail_opt, tail_source = build_run_objects(
        tail_args, dataset, device
    )
    evaluated = {}
    ...
    def evaluate_tail(df, decay_role):
        ...
        tail_metrics = run_loop(
            ...
            eval_interval=max(
                1,
                min(tail_args.eval_interval, tail_steps // 3 if tail_steps >= 3 else 1),
            ),
        ...
        row = {
            ...
            "stable": (not tail_metrics["diverged"])
            and math.isfinite(tail_metrics["final_val"]),
    ...
    for _ in range(max(0, args.max_extra_decay_evals)):
        next_df = pick_extra_decay_frac(
            decay_fracs, evaluated, args.decay_refine_rel_tol
        )
        if next_df is None:
            break
...
def summarize_branch_rows(rows):
    ...
    for key, group in by_pair.items():
        finals = [
            r["tail_final_val"] for r in group if math.isfinite(r["tail_final_val"])
        ]
    ...
    for lr_key, group in by_lr.items():
        ...
        if not scored:
            continue
        scored.sort(
            key=lambda g: score_key(g["score"], g["support_score"], g["exp2_lr"])
        )
        best_pair = scored[0]
        rep = dict(best_pair)
        rep.update(
...
def choose_top_lrs(lr_summaries, k: int):
    ...
    finite.sort(
        key=lambda r: score_key(
            r["potential_score"], r["potential_support_score"], r["exp2_lr"]
        )
    )
    return finite[:k]
...
def run_for_prenorm(prenorm, target_cfg, args, rest):
    ...
    screen_cache = auto_expand_screen(
        screen_args, dataset, device, amp_dtype, screen_seed, args
    )
    screen_rows = adaptive_zoom_screen(
        screen_cache, screen_args, dataset, device, amp_dtype, screen_seed, args
    )
    shortlist = shortlist_from_screen(
        screen_rows, args.shortlist_k, args.keep_highest_k, args.aggressive_lr_tol
    )
    ...
    race_rows = [
        run_constant_candidate(
            "trunk_race", exp2_lr, race_args, dataset, device, amp_dtype, race_args.seed
        )
        for exp2_lr in shortlist_exp2s
    ]
    finalists = choose_branch_finalists(
        race_rows, args.final_lr_k, args.aggressive_lr_tol
    )
    ...
    anchor_fracs = (
        parse_float_list(args.decay_anchor_fracs) if args.decay_anchor_fracs else []
    )
    ...
    while seeds_used < args.confirm_max_seeds:
        ...
        if (
            rel_gap(winner["potential_score"], runner["potential_score"])
            > args.confirm_rel_tol
        ):
            break
    ...
    for row in summary_rows:
        mT, mL, per_group = transfer_lrs(
            row["lr"], proxy_cfg, target_cfg, args.alpha_transfer
        )
        row["mT"] = mT
        row["mL"] = mL
...
def main():
    ...
    p.add_argument(
        "--cpu-snapshots",
        action="store_true",
        help="store branch snapshots on CPU instead of the active device",
    )
    ...
    write_csv(
        prefix.with_name(prefix.name + "_final_pair_summary.csv"),
        all_final_pair_summaries,
    )
    write_csv(
        prefix.with_name(prefix.name + "_final_lr_summary.csv"), all_final_lr_summaries
    )
